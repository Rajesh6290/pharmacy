...
import { LayoutGrid, Pill, ShoppingCart, ClipboardList, X } from "lucide-react";
...
const storeNav = [
  { label: "Browse All", href: "/store", icon: <LayoutGrid size={17} /> },
  { label: "Products", href: "/products", icon: <Pill size={17} /> },
  { label: "Cart", href: "/cart", icon: <ShoppingCart size={17} /> },
  { label: "My Orders", href: "/users", icon: <ClipboardList size={17} /> },
];
    ...
    <>
      ...
      >
        <div className="border-accent-200 flex h-16 items-center justify-between border-b px-5">
          <span className="text-accent-700 text-sm font-semibold">Store</span>
          <button
            onClick={onClose}
